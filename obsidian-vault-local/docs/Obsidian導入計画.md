...
1. **設定パターンライブラリ（Configuration Patterns）**
...
|---------|------|--------------|---------|---------|--------|
| **Trigger 1** | **学習開始（Phase 1全体）** | **学習セッション開始時** | **60%** | **AI概念説明 + 2段階検証（自己確認+AI確認）+ Phase 2移行判定** | **★★★** |
...
#### 4.1.2 週次Offset Map参照

**最新版の週次Offset Mapは以下を参照**:

- **6週プラン**: `docs/main/6週再編/6週プラン/6週プラン.md` L88-140
- **CLAUDE.md**: L59-111

> **Note**: 2025-12-09にoffset値を更新。古いコピーは削除し、Single Source of Truth原則に従い参照リンクに変更。
...
### 4.2 Trigger 1: 学習開始（Phase 1全体）

**検出キーワード**: `学習開始`

**目的**: Phase 1（AI説明・概念理解）全体をカバー。AI概念説明 → 理解度チェックリスト確認 → Phase 2移行判定まで実施

**自動化率**: 70%（AI説明は自動、ユーザー対話・チェックリスト確認あり）

**実行フロー**:

```
1. コンテキスト読み込み（完全自動）
   - progress_state.yaml: current_week, current_day, current_learning_item取得
   - W6_PLAN_DAY_MAP使用: 6週プラン当日セクション読み込み
...
4. 理解度チェックリスト確認（2段階検証・順次進行）
   ...
   【重要】Step A → Step B は必ず両方実施（Step Aの結果に関係なくStep Bへ進む）
...
5. 判定基準（Step B完了後に判定・2条件AND）
   - ✅ Step A (3+個) AND Step B (AI承認) → Phase 2へ移行可
   - ❌ どちらか未達 → 復習ループ → Step A/B再実施

6. progress_state.yaml更新
   - current_learning_item更新
   - セッション開始時刻記録
```
...
| メタデータ（count/occurrences/kb_promoted） | progress_state.yaml | 自動集計・判定 | 自動（T      rigger 7） |
...
> **設計根拠**: 「実装開始トリガー」と「集計トリガー」は処理が単純なため、実装時の確認項目を最小化し、開発効率を優先。
>
> **Note**: Trigger 1（学習開始）は v2.1 で Phase 1 全体（AI概念説明 + チェックリスト確認）に拡張されたため、対象外リストから除外。
