...
- **例外チェーン**: 例外を再ラップする際は原則 `raise ... from e` で原因を保持すること。ただし、外部の実在 API のレスポンス由来例外を未サニタイズのまま cause にしてはならない。§1「ログ・例外への露出禁止」(🔴 Blocking) を本項 (🟡 Suggested) より優先する。
  `from None` の対象は `utils/github_client.py` の `AsyncGitHubClient._request`、`utils/github_error_handler.py` の `_handle_403_response` / `_handle_5xx_response` / `_handle_http_status_error`、`utils/github_etag_cache.py` の `GitHubETagCache._cache_key`、`utils/response_parsing.py` の `validate_parsed_model` / `validate_parsed_model_list` である。
  サニタイズ済み代理 cause は `utils/github_error_handler.py` の `SanitizedJSONDecodeError` を使う。`utils/jsonplaceholder_client_async.py` の `AsyncJSONPlaceholderClient.get_user_data` と `config/settings.py` の `Settings.validate_environment` にある `from None` は PII 判断とは別理由であり、`from e` への統一を提案しない。判断基準は README「設計判断」の `例外チェーン方針` を参照する。
