...
LOCK_DIR="$CGC_STATE_DIR/.lock_cgc_watch_safe"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  OLD_PID="$(cat "$LOCK_DIR/pid" 2>/dev/null || true)"
  if [[ -n "$OLD_PID" ]] && kill -0 "$OLD_PID" 2>/dev/null; then
    echo "[CGC Patch] cgc_watch_safe already running (pid=$OLD_PID), exiting."
    exit 0
  fi
  rm -rf "$LOCK_DIR"
  mkdir "$LOCK_DIR"
...
# 1. Prepare local tree-sitter environment
export PYTHONPATH="$GEMINI_TMP:$PYTHONPATH"

# 2. Start cgc watch via python to apply patches and handle missing languages
python -c "
import sys
import os
import threading
...
libs_path = Path(r'$LIBS_PATH')
if libs_path.exists():
    try:
        sys.path.insert(0, r'$GEMINI_TMP')
        from tree_sitter_language_pack import configure
        configure(cache_dir=str(libs_path))
        print(f'[CGC Patch] tree-sitter-libs configured to: {libs_path}')
    except Exception as e:
        print(f'[CGC Patch] Failed to configure libs: {e}')

# --- Monkeypatch TreeSitterManager to handle missing languages gracefully ---
from codegraphcontext.utils import tree_sitter_manager
original_get_language = tree_sitter_manager.get_tree_sitter_manager().get_language_safe

def patched_get_language_safe(lang):
    try:
        return original_get_language(lang)
    except (ValueError, Exception) as e:
        print(f'[CGC Patch] Skipping unsupported language \"{lang}\": {e}')
        # Return a dummy or just re-raise if it's not python
        if lang in ('python', 'py'):
            raise e
        return None

# Use a simpler patch: just ignore failures for non-essential languages in GraphBuilder
...
def patched_parser_init(self, language_name):
    try:
        ...
        if language_name in ('python', 'py'):
            raise e
        print(f'[CGC Patch] Disabled parser for \"{language_name}\" due to: {e}')
        self.language = None
        self.parser = None
...
try:
    ...
    print('[CGC Patch] watchdog Observer forced to PollingObserver')
except Exception as e:
    print(f'[CGC Patch] Failed to force PollingObserver: {e}')
...
if watch_mode == 'incremental':
    try:
        ...
        print(f'[CGC Patch] Failed to patch incremental refresh: {e}')
else:
...
sys.argv = ['cgc', 'watch', '.']
try:
    print('[CGC Patch] Starting Live Sync...')
    app()
except SystemExit:
    pass
"
