...
Python 3.13ベースのAPIテスト + DevOps統合プロジェクト。同期/非同期HTTPクライアント、型安全な設定管理、包括的テストスイートを特徴とする。
...
### プログラミング言語
- **Python**: 3.13（実装環境）
  - CI/CDでは3.13固定（pyproject.toml: `requires-python = "==3.13.*"`）
