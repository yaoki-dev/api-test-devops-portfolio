...
def test_jitter_consistency_across_attempts():
    ...
    - 誕生日のパラドックスによる期待ユニーク数:
        - attempt=0: range=0.6秒(60値) → 期待45-50個 → 基準40個
        - attempt=1: range=1.2秒(120値) → 期待63-68個 → 基準60個
        - attempt=2: range=2.4秒(240値) → 期待79-84個 → 基準75個
        - attempt=3: range=4.8秒(480値) → 期待89-92個 → 基準85個
    """
    attempts_config = [
        (0, 40),  # attempt=0: 基準40個（統計的期待値45-50個）
        (1, 60),  # attempt=1: 基準60個（統計的期待値63-68個）
        (2, 75),  # attempt=2: 基準75個（統計的期待値79-84個）
        (3, 85),  # attempt=3: 基準85個（統計的期待値89-92個）
    ]
    base_delay = 1.0
