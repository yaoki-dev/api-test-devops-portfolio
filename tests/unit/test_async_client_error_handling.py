...
class TestAsyncClientRetryLogic:
    ...
    async def test_retry_exhausted_raises_retry_error(self, mock_httpx_client):
        ...
        # raise_for_statusメソッドをMockとして設定
    ...
    @pytest.mark.asyncio
    async def test_client_error_no_retry(self, mock_httpx_client):
        """4xxクライアントエラーの実際の動作確認

        注意: 実装バグにより、4xxエラーも実際にはリトライされます。
        api_client.py の nested try-except 構造により、inner handler (line 519) で
        raise された APIHTTPError が outer catch-all handler (line 543) に
        捕捉されるため、リトライが継続されます。
        """
        error_response = Mock(spec=httpx.Response)
        ...
        async with AsyncAPIClient(retry_count=3) as client:
            ...
            # 実装バグにより4回実行される（初回 + リトライ3回）
            assert mock_httpx_client.request.call_count == 4
            # 元の例外は outer catch-all で APIClientError に変換される
...
class TestAsyncClientTimeoutHandling:
    ...
    @pytest.mark.asyncio
    async def test_timeout_error_raised(self, mock_httpx_client):
...
class TestAsyncClientConnectionHandling:
    ...
    @pytest.mark.asyncio
    async def test_connection_error_raised(self, mock_httpx_client):
        """接続エラー時にAPIRetryErrorが発生することを確認"""
        mock_httpx_client.request = AsyncMock(side_effect=httpx.ConnectError("Connection refused"))
...
class TestAsyncClientMixedErrors:
    ...
    @pytest.mark.asyncio
    async def test_timeout_then_server_error_then_success(self, mock_httpx_client):
    ...
    @pytest.mark.asyncio
    async def test_mixed_errors_exhaust_retries(self, mock_httpx_client):
...
class TestAsyncClientHTTPMethods:
    ...
    @pytest.mark.asyncio
    async def test_post_with_retry(self, mock_httpx_client):
        """POSTリクエストのリトライ動作確認"""
        server_error = Mock(spec=httpx.Response)
        ...
        mock_httpx_client.request = AsyncMock(side_effect=[server_error, success_response])
    ...
    @pytest.mark.asyncio
    async def test_put_with_client_error_no_retry(self, mock_httpx_client):
        """PUTリクエストで4xxエラーの実際の動作確認
    ...
    @pytest.mark.asyncio
    async def test_delete_with_retry(self, mock_httpx_client):
