...
**🔴 MUST（作業開始時に必読）**:
- `coding_standards` - コーディングルール違反防止（命名規則、型ヒント、テスト規約）
...
型エラー時 → @memory:coding_standards Section 2-3（型ヒント規約）
...
**🔴 MUST（作業開始時に必読）**:
- `test_strategy` - テスト戦略全体像（4層アーキテクチャ、7種類マーカー）
...
- `coding_standards` - テスト規約確認（フィクスチャ、モック規約）
...
pytest並列実行エラー → @memory:test_strategy Section 4（トラブルシューティング）
```
...
- `coding_standards` - ドキュメント規約（最終更新日記載等）
...
| **型エラー** | coding_standards | Section 2-3 | 型ヒント規約、PascalCase/snake_case |
...
| **並列テストエラー** | test_strategy | Section 4 | トラブルシューティング |
...
→ @memory:coding_standards（命名規則、型ヒント確認）
...
【Phase 1: MUST自動読み込み】
作業開始時（0.8秒、1,000トークン）:
→ @memory:test_strategy（全体戦略、4層アーキテクチャ）
...
```python
# ❌ 非効率: Read tool直接呼び出し
Read(".serena/memories/coding_standards.md", limit=100)
# → 2,000トークン消費、最適化なし
...
```python
# ❌ 逐次読み込み（4.5秒）
Read("coding_standards.md")       # 1.5秒
Read("implementation_quality_gates.md")  # 1.5秒
Read("project_architecture.md")   # 1.5秒

# ✅ 並列読み込み（1.5秒、66%時間削減）
Read("coding_standards.md") | Read("implementation_quality_gates.md") | Read("project_architecture.md")
```

### 3. 部分読み込み（大ファイル対策）

```python
# test_strategy.md（全体300行）の部分読み込み
Read("test_strategy.md", offset=0, limit=50)    # Section 1-2のみ
Read("test_strategy.md", offset=100, limit=50)  # Section 5のみ
...
```python
# 同一セッション内での再読み込み回避
# 1回目: @memory:coding_standards（1,000トークン）
# 2回目: キャッシュヒット（0トークン）
```
