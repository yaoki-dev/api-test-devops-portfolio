...
conftest.py（397行）から抽出した16フィクスチャのクイックリファレンス。新規テスト作成時の効率的なフィクスチャ選択を支援。
...
|--------------|---------|------|------|--------|
| `test_config` | session | API設定（base_url, timeout等） | - | L73 |
| `logger` | function | テスト用ロガー | - | L91 |
...
| フィクスチャ名 | スコープ | 用途 | 依存 | 行番号 |
|--------------|---------|------|------|--------|
| `async_client` | function | 実際のAsyncAPIClientインスタンス（※非同期） | test_config | L110 |
| `mock_httpx_client` | function | モック化httpx.AsyncClient | - | L123 |
| `sample_api_response` | function | サンプルJSONレスポンス | - | L135 |
| `mock_successful_response` | function | 成功レスポンス（200） | sample_api_response | L141 |
| `mock_error_response` | function | エラーレスポンス（404） | - | L153 |

### 1.3 テストデータファクトリー

| フィクスチャ名 | スコープ | 用途 | パラメータ例 | 行番号 |
|--------------|---------|------|-------------|--------|
| `todo_data_factory` | function | TODOデータ生成 | `user_id`, `completed` | L172 |
| `user_data_factory` | function | ユーザーデータ生成 | `name`, `email` | L192 |
| `post_data_factory` | function | 投稿データ生成 | `title`, `body` | L226 |

### 1.4 専門テスト用フィクスチャ

| フィクスチャ名 | スコープ | 用途 | 対象テスト | 行番号 |
|--------------|---------|------|-----------|--------|
| `performance_timer` | function | 実行時間計測タイマー | @pytest.mark.performance | L246 |
| `error_scenarios` | function | エラーシナリオ辞書 | エラーハンドリング | L282 |
| `security_payloads` | function | 攻撃ペイロード辞書 | セキュリティテスト | L300 |
| `integration_test_data` | function | 大量テストデータセット | @pytest.mark.integration | L332 |
...
| フィクスチャ名 | スコープ | 用途 | 行番号 |
|--------------|---------|------|--------|
| `cleanup_test_files` | function | テンポラリファイル削除 | L358 |
| `reset_settings` | function | 設定リロード（テスト独立性保証） | L369 |
...
async def test_async_api_with_mock(mock_httpx_client):
    ...
    # conftest.py L127: mock_client.get = AsyncMock() 済み
    mock_httpx_client.get.return_value = Mock(
        status_code=200,
...
|---------|------|--------|
| `unit` | 単体テスト | L47 |
| `integration` | 統合テスト | L48 |
| `e2e` | E2Eテスト | L49 |
| `slow` | 実行時間の長いテスト | L50 |
| `external` | 外部API依存テスト | L51 |
| `performance` | パフォーマンステスト | L52 |
| `smoke` | スモークテスト（main PR用） | L53 |
...
- **conftest.py**: `tests/conftest.py`（397行）
