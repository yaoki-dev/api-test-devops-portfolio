...
**自リポジトリの issue / PR 番号はコードに書かない。**

責務分離のため。issue / PR は「変更の経緯」を、コードは「現在の仕様とその理由」を説明する。
番号参照は読み手に番号を追う作業を強いるうえ、経緯自体は `git blame` と PR 履歴から
復元できるためコード内では冗長になる。番号ではなく理由そのものを書く。

```python
# 悪い例: 番号を追わないと理由が分からない
# キャッシュ更新失敗時は再 raise する（PR#123 B-3）

# 良い例: その場で理由が完結する
# キャッシュ更新失敗は握り潰さず再 raise する。
# 握り潰すと次回リクエストが不整合なキャッシュを参照するため。
```

- **適用範囲**: 本番ソース（`utils/` `config/` `models/`）とテストコード（`tests/`）の両方
- **対象外 — 外部リポジトリの issue / バグ ID は記載してよい**（例: `encode/httpx#3221`、`octokit issue #566`）
  - 外部の不具合 ID は「なぜこの workaround が必要か」「upstream 修正後に撤去してよいか」を示す
    現在の仕様の根拠であり、自リポジトリの履歴からは復元できない。自リポ参照が冗長なのに対し、
    外部参照は非冗長
- **遡及適用しない**: 本規約の追加時点で既に存在する記載は書き換えない。新規追加・変更するコードにのみ適用する
