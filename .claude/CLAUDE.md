...
14. **ALWAYS** enforce worktree boundary:
    ...
    - Run `git worktree list --porcelain` (via pipeline) and check result:
      ...
      > **Free text rules** (2+ entries scope confirmation only): interpret user input — (a) input contains an absolute path (starts with `/`): treat as worktree path specification → **STOP** + report specified path + instruct session restart at correct worktree; (b) input matches common approval patterns ("yes", "proceed", "ok", "承認", etc.): treat as Approve → **continue**; (c) all other cases: treat as ambiguous → use AskUserQuestion tool to re-ask with closed-list options ["Approve and continue" / "Reject and stop"] — silent continuation is NEVER permitted
