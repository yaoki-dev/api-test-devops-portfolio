...
14. **ALWAYS** enforce worktree boundary:
    ...
    - Run `git worktree list` and check result:
      ...
      > **WORKTREE_ROOT確認**: WORKTREE_ROOTが非空であることを先に確認（空の場合は **STOP**）。非空の場合: `git worktree list | cut -d' ' -f1 | grep -Fx "${WORKTREE_ROOT}"` (`-F`: literal string, `-x`: full-line match — prevents `/project` matching `/project-extra`; note: paths with spaces are not supported by this command)
      > **「パース不可」の定義**: **いずれかの**行が `<absolute-path> <hash> <branch-info>` の3フィールド形式を満たさない（1行でも不正 → STOP）/ `cut -d' ' -f1` で取り出したフィールドが絶対パスでない（`/`で始まらない）/ 出力が空白のみ
...
| ID | Time | T | Title | Read |
|----|------|---|-------|------|
| #13031 | 9:36 AM | 🔵 | Plan Executability Evaluation Completed - Barriers and Recommendations Identified | ~611 |
</claude-mem-context>
