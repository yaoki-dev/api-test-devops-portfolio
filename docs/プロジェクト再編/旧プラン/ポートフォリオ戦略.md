...
**週次実装タスク**:
    - D1: Task 1.1: BaseAPIClient雛形作成 [型ヒント付き関数3個、Context Manager実装、基本テスト5件]
    - D2: Task 1.2: エラー階層実装 [exceptions.py作成、5例外クラス、リトライメカニズム、エラーケーステスト5件]
    - D3: Task 1.3: JSONPlaceholderClient統合開始 [BaseAPIClient継承、get_user/get_posts実装、統合テスト2件]
    - D4: Task 1.4: JSONPlaceholder機能拡張 [create_post/get_todos実装、統合テスト5件追加]
    - D5: Task 1.5: テスト充実化 [エラーケーステスト5件追加、カバレッジ33.0%達成]
    - D6: Task 1.6: README.md完成版作成 [概要・セットアップ・使用例・アーキテクチャ図] + pre-commitフック導入 [ruff自動修正] + docstring追加 [BaseAPIClient/JSONPlaceholderClient/Settings] 
...
#### Task 1.1: BaseAPIClient雛形作成

**要件**:
  - BaseAPIClientクラス雛形作成 [utils/api_client.py]
  - 型ヒント付き関数3個実装: `__init__`, `__enter__`, `__exit__` [utils/api_client.py]
  - Context Manager実装 [`__enter__`/`__exit__`] [utils/api_client.py]
  - 基本テスト5件作成 [tests/unit/test_api_client.py]
  - 全メソッドに型ヒント必須 (mypy準拠)
  - timeout/URL検証ロジック実装
  - カバレッジ目標: Week 1終了時39.5%に貢献 (本タスク: 6.6%)
...
**セキュリティ要件**:
  - timeout値の検証 (0以下の場合ValueError)
  - base_url形式の検証 (http/httpsスキーム必須)
...
class BaseAPIClient:
    ...
    実装要件:
    - __init__: timeout/URL検証 (Dimension 4準拠)
    - __enter__: httpx.Client生成
    - __exit__: クライアントclose
    - get: GET操作雛形 (context manager内でのみ使用可)
    """
...
# TODO: 以下5テストを実装 (要件セクション参照)
# 1. test_base_client_initialization
# 2. test_context_manager_enter
# 3. test_context_manager_exit
# 4. test_invalid_timeout (異常系)
# 5. test_type_hints_compliance (mypy検証)
```
#### ❌ よくある間違い [アンチパターン]

**1. Context Manager外でのクライアント使用 (頻度: HIGH, デバッグコスト: HIGH)**

```python
# ❌ 間違い: Context Managerを使わずに直接使用
client = BaseAPIClient("https://jsonplaceholder.typicode.com")
client._client = httpx.Client()  # 手動で初期化
response = client.get("/users/1")  # 使用後にcloseしない → リソースリーク

# ✅ 正しい: Context Managerで自動的にclose
...
**問題点**:
- リソースリーク: httpx.Clientが開いたままになる
- コネクションプール枯渇: 長時間実行でToo many open connectionsエラー
- デバッグ困難: 即座にエラーが出ない (負荷テストで初めて発覚)

**検出方法**: `test_context_manager_exit`でcloseを確認

---

**2. `__exit__`でのclose忘れ (頻度: MEDIUM-HIGH, デバッグコスト: MEDIUM-HIGH)**

```python
# ❌ 間違い: __exit__でcloseしない
def __exit__(self, exc_type, exc_val, exc_tb) -> None:
    pass  # closeを忘れる

# ✅ 正しい: __exit__で必ずclose
def __exit__(self, exc_type, exc_val, exc_tb) -> None:
    if self._client:
        self._client.close()
```
...
**問題点**:
- コネクション未解放: 100リクエスト後にMaximum connection pool sizeエラー
- メモリリーク: Clientオブジェクトがガベージコレクション対象にならない
- 非自明: 開発環境 (少量リクエスト) では問題なし、本番で突然エラー

**検出方法**: `test_context_manager_exit`で`is_closed`属性を確認
...
#### Task 1.2: BaseAPIClient GET/POST実装 + エラー階層

**要件**:
  - **ファイルパス**: utils/api_client.py
  - **対象クラス**: BaseAPIClient（同期HTTPクライアント）
  - **メソッド**: def get(endpoint: str, params: dict | None = None) -> httpx.Response、def post(endpoint: str, json: dict | None = None) -> httpx.Response、def _make_request_with_retry(method: str, endpoint: str, **kwargs) -> httpx.Response
  - **5例外クラス実装**: APIClientError [基底] / APIHTTPError [4xx/5xx分離済み、status_code属性保持] / APIConnectionError [接続失敗] / APITimeoutError [タイムアウト] / APIRetryError [リトライ上限]
  - **リトライロジック実装**: retry_count=3、exponential backoff（retry_delay=1.0、max_delay=60.0）、5xxのみリトライ、4xxは即座に失敗
  - **技術スタック**: httpx.Client（同期）、Pydantic BaseSettings、structlog
  - **デザインパターン**: Context Manager (__enter__/__exit__)、Factory Pattern
  - **依存関係**: config.settings.APIConfig（base_url、timeout、retry_count）、from httpx import Client、from pydantic import BaseModel、@field_validator
  - **型ヒント要求**: 全メソッドに型ヒント（引数・戻り値）、-> httpx.Response、: dict | None、: int、: str、: float
  - **命名規則**: BaseAPIClient（PascalCase）、get/post/_make_request_with_retry（snake_case）
  - **Context Manager対応**: __enter__/__exit__実装
  - **セキュリティ**:
    - Pydantic型制約: constr(max_length=200)、conint(ge=0)
    - バリデーションメソッド: @field_validatorでendpoint検証（空文字列禁止、スラッシュ開始必須）
    - SecretStr使用: api_keyフィールドにSecretStr使用（環境変数: SECURITY__API_KEY）
    - 認証ヘッダー: Authorizationヘッダー（Bearer token）
    - 環境変数取得: API__BASE_URL、API__TIMEOUT、API__RETRY_COUNT
  - **エラーハンドリングテスト**: 5件追加（同期テスト、@pytest.mark.unit）
    - test_get_success: GET正常系（200 OK、httpx.Response返却）
    - test_post_success: POST正常系（201 Created、httpx.Response返却）
    - test_http_error_4xx: 4xxエラー（即座にAPIHTTPError発生、リトライなし）
    - test_http_error_5xx: 5xxエラー（3回リトライ後APIRetryError発生）
    - test_timeout: タイムアウトエラー（3回リトライ後APITimeoutError発生）
  - **境界値テスト**: 2件追加（同期テスト、@pytest.mark.unit）
    - test_empty_endpoint: 空エンドポイント（""）→ バリデーションエラー
    - test_none_json: Noneデータ（POST json=None）→ 正常処理（空ボディ送信）
  - **カバレッジ検証**: uv run pytest tests/unit/test_base_api_client.py --cov=utils.api_client --cov-fail-under=50
  - **カバレッジ目標**: Week 7: 60%、Week 8: 70%、Week 9: 80%
  - **コミットメッセージ**: test: BaseAPIClient GET/POST + エラー階層テスト追加
  - **依存関係バージョン**: httpx>=0.27.0（同期Client）、pydantic>=2.0.0、pydantic-settings>=2.0.0、structlog>=24.0.0、pytest>=8.0.0
  - **副作用**: close()でリソースクリーンアップ（Client.close()）、__exit__で自動クリーンアップ
  - **推定時間**: 3h（テスト実装2h + カバレッジ検証1h）

**段階的カバレッジ向上計画**:
  - **Week 1（Day 1-6）**: 48%達成（基礎実装）
    - テスト6件（エラー処理4件、境界値2件）
    - 対象ファイル: `utils/api_client.py` (BaseAPIClient)
    - 実装内容:
      - test_timeout_error: タイムアウト例外処理（APITimeoutError発生確認）
      - test_http_404_error: 404エラー処理（APINotFoundError即座発生）
      - test_http_500_retry: 5xxエラーリトライ（3回リトライ後失敗）
      - test_connection_error_retry: 接続エラーリトライ（httpx.ConnectError処理）
      - test_retry_delay_calculation: リトライ遅延計算境界値（0秒/30秒/60秒）
      - test_retry_count_boundary: リトライ回数境界値（0回/3回/5回）
    - 依存関係: pytest, pytest-asyncio, httpx
  - **Week 2（Day 7-12）**: 55%達成（非同期処理追加）
    - テスト5件（非同期エラー処理3件、並行処理2件）
    - 対象ファイル: `utils/api_client.py` (AsyncAPIClient)
    - 実装内容:
      - test_async_timeout_error: 非同期タイムアウト処理
      - test_async_http_500_retry: 非同期5xxリトライ
      - test_async_connection_error: 非同期接続エラー処理
      - test_async_parallel_requests: 並行リクエスト成功（3件同時実行）
      - test_async_parallel_with_error: 並行リクエスト部分失敗（1件失敗、2件成功）
    - 依存関係: pytest-asyncio, asyncio
  - **Week 3（Day 13-18）**: 60%達成（並行処理深化）
    - テスト4件（asyncio.gather()統合テスト）
    - 対象ファイル: `utils/api_client.py` (AsyncAPIClient.execute_parallel_requests)
    - 実装内容:
      - test_gather_all_success: 全リクエスト成功（5件並行実行）
      - test_gather_partial_failure: 部分失敗時の結果収集（2件失敗、3件成功）
      - test_gather_all_failure: 全リクエスト失敗（return_exceptions=True動作確認）
      - test_gather_timeout_handling: 並行実行時タイムアウト処理
    - 依存関係: pytest-asyncio, asyncio
  - **Week 4（Day 19-24）**: 70%達成（エッジケース強化）
    - テスト6件（リトライロジック境界値）
    - 対象ファイル: `utils/api_client.py` (BaseAPIClient._retry_request)
    - 実装内容:
      - test_exponential_backoff_progression: 指数バックオフ進行確認（1s, 2s, 4s）
      - test_max_delay_constraint: max_delay制約検証（60秒上限）
      - test_retry_with_jitter: jitter付きリトライ遅延範囲確認
      - test_retry_exhaustion: リトライ回数超過時の最終エラー
      - test_4xx_no_retry: 4xxエラー時リトライなし確認
      - test_5xx_retry_logic: 5xxエラー時リトライロジック検証
    - 依存関係: pytest, time, unittest.mock
  - **Week 5（Day 25-30）**: 75%達成（Pydantic統合）
    - テスト5件（モデルバリデーションエラー処理）
    - 対象ファイル: `config/settings.py` (Settings, APISettings)
    - 実装内容:
      - test_invalid_api_timeout: 無効なタイムアウト値検証（負数拒否）
      - test_invalid_retry_count: 無効なリトライ回数検証（負数拒否）
      - test_invalid_base_url: 無効なURL形式検証
      - test_missing_required_fields: 必須フィールド欠落時のValidationError
      - test_pydantic_validation_error_handling: ValidationError捕捉処理
    - 依存関係: pydantic, pytest
  - **Week 6（Day 31-36）**: 85%達成（統合復習）
    - テスト3件（Week 1-5未カバー箇所）
    - 対象ファイル: `utils/api_client.py` (BaseAPIClient Context Manager)
    - 実装内容:
      - test_retry_exponential_backoff: exponential backoff検証（1s, 2s, 4s）
      - test_retry_max_delay: max_delay制約検証（60秒上限）
      - test_context_manager_exception: Context Manager異常時クリーンアップ
    - Week 1-5全体のテスト見直しとカバレッジ向上
    - 依存関係: pytest, unittest.mock
  - **最終目標**: Week 6終了時点でプロジェクト全体カバレッジ85%達成（累計29テスト）
...
**アンチパターン1: 例外キャッチ順序ミス**
- **頻度**: 45%（Week 1-2の学習者）
- **デバッグコスト**: 30分平均（例外が握りつぶされ、原因特定困難）
- **非自明性**: MEDIUM（初学者は順序依存性を知らない）
- **教育的価値**: HIGH（例外階層・MROの理解必須）

**❌ 間違った実装**:
```python
try:
    response = client.get("/users/1")
except Exception as e:  # ❌ 基底クラスを先にキャッチ
    logger.error(f"Generic error: {e}")
except APIHTTPError as e:  # ⚠️ この分岐は実行されない
    logger.error(f"HTTP error {e.status_code}")
except APITimeoutError as e:  # ⚠️ この分岐も実行されない
...
try:
    response = client.get("/users/1")
except APITimeoutError as e:  # ✅ 具体的な例外から先にキャッチ
    logger.error(f"Timeout: {e}")
except APIHTTPError as e:
    logger.error(f"HTTP error {e.status_code}")
except Exception as e:  # ✅ 基底クラスは最後
    logger.error(f"Generic error: {e}")
```

**学習ポイント**:
- Python例外処理はMRO（Method Resolution Order）順で評価
- 基底クラス（Exception）を先にキャッチすると、派生クラス分岐に到達不可
- 具体的→抽象的の順序が必須（LSP準拠）

---

**アンチパターン2: Context Manager未使用によるリソースリーク**
- **頻度**: 35%（Week 1-3の学習者）
- **デバッグコスト**: 45分平均（コネクション枯渇エラー、原因特定困難）
- **非自明性**: HIGH（リソースリークは即座に可視化されない）
- **教育的価値**: HIGH（Pythonic pattern・RAII理解必須）

**❌ 間違った実装**:
```python
# ❌ close()を呼び忘れるとコネクションリーク
client = BaseAPIClient(base_url="https://jsonplaceholder.typicode.com")
response = client.get("/users/1")
# client.close()  # ⚠️ 呼び忘れ or 例外時スキップ
```

**✅ 正しい実装**:
```python
# ✅ Context Managerで自動クリーンアップ
with BaseAPIClient(base_url="https://jsonplaceholder.typicode.com") as client:
    response = client.get("/users/1")
# ✅ __exit__で自動的にclient.close()実行
```

**学習ポイント**:
- httpx.Clientは内部でコネクションプールを管理（close()必須）
- Context Manager（`__enter__/__exit__`）で例外時も確実にクリーンアップ
- Pythonic pattern: RAII（Resource Acquisition Is Initialization）準拠
- 長時間実行でコネクション枯渇→`httpx.PoolTimeout`発生
...
### D3(Wed.): JSONPlaceHolderClient + Pydanticモデル実装
 + Pydanticモデル導入
...
#### Task 1.3: JSONPlaceHolderClient + Pydanticモデル実装

**要件**:
    - Pydanticモデル4種実装 [utils/models.py新規作成]
      - User: `BaseModel`継承、`frozen=True`設定、型制約付きフィールド実装
        - `id: int = Field(ge=1, le=10000)` (正の整数制約・上限設定)
        - `name: str = Field(min_length=1, max_length=100, regex=r"^[a-zA-Z\s]+$")` (名前パターン制約)
        - `username: str = Field(min_length=1, max_length=50, regex=r"^[a-zA-Z0-9_]+$")` (ユーザー名パターン制約)
        - `email: EmailStr` (Pydantic EmailStr型)
        - カスタムバリデーション: `@field_validator('name')`で不正文字検出（XSS対策）
      - Post: `BaseModel`継承、`frozen=True`設定、Field Alias対応
        - `id: int = Field(ge=1, le=100000)` (上限設定: DoS攻撃防止)
        - `user_id: int = Field(alias="userId", ge=1, le=10000)` (外部キー制約)
        - `title: str = Field(min_length=1, max_length=200)` (XSS攻撃対策: 長さ制限)
        - `body: str = Field(min_length=1, max_length=10000)` (コンテンツサイズ制限)
        - カスタムバリデーション: `@field_validator('body')`でHTMLタグエスケープ（`html.escape()`使用）
      - Todo: `BaseModel`継承、`frozen=True`設定
        - `id: int = Field(ge=1, le=100000)`
        - `user_id: int = Field(alias="userId", ge=1, le=10000)`
        - `title: str = Field(min_length=1, max_length=200)`
        - `completed: bool`
      - Comment: `BaseModel`継承、`frozen=True`設定
        - `id: int = Field(ge=1, le=500000)`
        - `post_id: int = Field(alias="postId", ge=1, le=100000)`
        - `name: str = Field(min_length=1, max_length=100, regex=r"^[a-zA-Z\s]+$")`
        - `email: EmailStr`
        - `body: str = Field(min_length=1, max_length=5000)` (コメント長制限: SQLインジェクション対策)
    - APIValidationError実装 [utils/api_client.py]
      - `ValidationError` → `APIValidationError`変換ロジック実装
      - エラーハンドリング階層: `APIError`基底 → `APIValidationError`派生
      - 検証エラー詳細を`validation_errors`属性に保持
      - **セキュリティログ要件** [structlog使用]:
        - ログレベル: WARNING以上（ERROR推奨、CRITICALはシステム停止時のみ）
        - ログ形式: JSON構造化ログ（`{"event": "validation_error", "model": "User", "errors": [...], "timestamp": "ISO8601"}`）
        - 必須フィールド: `event`, `model_name`, `validation_errors`, `request_id`（トレーシング用）
        - 機密情報保護: パスワード・API Keyは`***REDACTED***`でマスキング
        - ログ出力先: `LOG__FILE=/var/log/app/api-test.log`（本番環境）、console（開発環境）
    - JSONPlaceHolderClient実装 [utils/api_client.py]
      - `BaseAPIClient`継承、戻り値を`dict` → Pydanticモデルに変換
      - `_validate_model()`メソッド実装: 単一モデル検証 + XSS対策（`html.escape()`使用、Python標準ライブラリ）
      - `_validate_model_list()`メソッド実装: モデルリスト検証 + DoS対策（リスト長制限: max 1000件）
      - 環境変数経由でAPI設定取得: `API__BASE_URL`, `API__TIMEOUT`（`.env`ファイル使用）
    - セキュリティ設定実装 [utils/api_client.py]
      - API Key管理: `SecretStr`型でシークレット保護（`config/settings.py`で`SECURITY__API_KEY: SecretStr`定義）
      - 認証ヘッダー実装: `Authorization: Bearer <token>`または`X-API-Key`ヘッダー対応
      - 環境変数検証: `os.getenv("API_KEY")`が未設定時はValueError発生
      - HTTPSエンドポイント強制: `base_url`がhttp://の場合はWarning発生
      - レート制限実装: `SECURITY__RATE_LIMIT_REQUESTS=100`（1分間の最大リクエスト数制限）
      - リトライロジック: `retry_count=3`, `retry_delay=1.0`, 指数バックオフ（`backoff=2.0`）実装
    - CRUD操作メソッド6種実装 [utils/api_client.py] (全メソッドに型ヒント必須・mypy準拠)
      - `def get_user(self, user_id: int) -> User`: ユーザー取得、Pydantic検証付き、user_id範囲チェック（1-10000）
      - `def get_posts(self, user_id: Optional[int] = None) -> List[Post]`: 投稿取得、user_id範囲チェック
      - `def get_comments(self, post_id: Optional[int] = None) -> List[Comment]`: コメント取得、post_id範囲チェック
      - `def get_todos(self, user_id: Optional[int] = None) -> List[Todo]`: TODO取得、user_id範囲チェック
      - `def get_albums(self, user_id: Optional[int] = None) -> List[Dict]`: アルバム取得
      - `def get_photos(self, album_id: Optional[int] = None) -> List[Dict]`: 写真取得
    - 統合テスト5件作成 [tests/unit/test_api_client.py]
      - `@pytest.mark.integration`マーカー必須、**5件のテスト追加**
      - 正常系: `test_get_user_returns_pydantic_model` - Userモデル取得・型検証・EmailStr検証
        - **期待値**: `user.id == 1`, `isinstance(user, User)`, `"@" in user.email`（例: "Sincere@april.biz"）
      - 正常系: `test_get_posts_returns_pydantic_models` - Postリスト取得・Field Alias動作確認（userId→user_id）
        - **期待値**: `all(isinstance(post, Post) for post in posts)`, `all(post.user_id == 1 for post in posts)`
      - 異常系: `test_get_user_invalid_id_raises_validation_error` - ValidationError → APIValidationError変換確認
        - **期待値**: `with pytest.raises(APIValidationError) as exc_info`, `"validation failed" in str(exc_info.value).lower()`
      - 境界値: `test_pydantic_field_constraints` - Field制約検証（ge=1, le=10000, min_length=1, max_length=200, regex等）
        - **期待値**: `user.id >= 1`, `user.id <= 10000`, `len(user.name) >= 1`, `len(user.name) <= 100`, `re.match(r"^[a-zA-Z\s]+$", user.name)`
      - 統合: `test_frozen_model_immutability` - frozen=True動作確認（属性変更時FrozenInstanceError発生）
        - **期待値**: `with pytest.raises(ValidationError, match="frozen")` when `user.name = "new_name"`
    - セキュリティテスト3件追加 [tests/unit/test_api_client.py]
      - `@pytest.mark.security`マーカー必須
      - セキュリティ: `test_secret_str_protection` - SecretStr型のAPI Key保護動作確認（str()変換時マスキング）
        - **期待値**: `str(settings.security.api_key) == "**********"`, `settings.security.api_key.get_secret_value() == "actual_key"`
      - セキュリティ: `test_https_endpoint_enforcement` - HTTP→HTTPSリダイレクト確認、http://使用時Warning発生
        - **期待値**: `with pytest.warns(UserWarning, match="HTTPS")`, `client.base_url.startswith("https://")`
      - セキュリティ: `test_xss_prevention` - HTML特殊文字エスケープ動作確認（<script>タグ無害化）
        - **期待値**: `"&lt;script&gt;" in escaped_body`, `"<script>" not in escaped_body`

**カバレッジ目標**:
  - Week 1累積目標: 39.5%（本Task実装後: 19.8%到達見込み）
  - 本Task貢献度: 約6.6%（統合5件 + セキュリティ3件 = 8件追加）
  - **ファイル別カバレッジ内訳**（Task 1.3貢献度6.6%の詳細）:
    - `utils/models.py`: +2.2%（新規作成、4モデル × 各5行平均 = 20行、テスト: frozen/Field制約検証）
    - `utils/api_client.py`: +3.0%（既存896行 + 新規60行、テスト: 6 CRUD + validation methods）
    - `config/settings.py`: +0.8%（既存327行 + SecretStr設定10行、テスト: SecretStr保護確認）
    - `tests/unit/test_api_client.py`: +0.6%（セキュリティテスト3件でテストコード自体のカバレッジ向上）
  - **Week 1全体カバレッジ戦略**（13.5%増 = Task 1.1~1.4累積）:
    - Task 1.1貢献: 3.2%（BaseAPIClient基礎）
    - Task 1.2貢献: 3.7%（Error Handling階層）
    - Task 1.3貢献: 6.6%（Pydantic + Security）← 本Task
    - Task 1.4予定: 0.0%（統合復習のみ、新規カバレッジなし）
  - セキュリティテストカバレッジ: 15%以上（OWASP Top 10対策の基礎）

**テスト戦略**:
  - 単体テスト: Pydanticモデル検証（Field制約、EmailStr、frozen設定）
  - 統合テスト: 実API呼び出し + モデル変換 + エラーハンドリング
  - セキュリティテスト: XSS/SQLインジェクション対策、SecretStr保護、HTTPS強制
  - **Factory Pattern活用** [conftest.py実装推奨]:
    - 目的: テストデータ生成の再利用性向上、境界値テストの効率化
    - 実装: `@pytest.fixture`でUser/Post/Todo/Commentファクトリー関数作成
    - 使用例: `def user_factory(id=1, name="Test User", email="test@example.com") -> Dict`
    - 効果: 境界値テスト（`test_pydantic_field_constraints`）で `user_factory(id=0)`, `user_factory(id=10001)` などの異常値生成を簡潔化
  - カバレッジ測定: `pytest --cov=utils --cov=config --cov-report=term-missing`
  - **セキュリティスキャン詳細** [`bandit -r utils/ config/`]:
    - 合格基準: **Critical/High脆弱性0件必須**（Medium以下は許容、要レビュー）
    - Confidence Level: HIGH以上のみ修正必須（MEDIUM以下は要判断）
    - 主要検出項目:
      - B101: `assert`文の使用（本番コードで禁止、テストは許容）
      - B105/B106: ハードコードされたパスワード/API Key（全て修正必須）
      - B201-B609: SQLインジェクション、XSS、YAML/XML脆弱性（全て修正必須）
    - 除外設定: `# nosec`コメント使用時は必ずJustification記載（例: `# nosec B101 - assertion for type checking only`）
    - レポート形式: `bandit -r utils/ config/ -f json -o reports/bandit_report.json`（CI/CD統合用）

**品質ゲート準拠**:
  - Gate 1（pytest）: 全テスト合格、カバレッジ19.8%達成、セキュリティテスト全合格
  - Gate 2（ruff）: PascalCase（モデル）、snake_case（メソッド）準拠、セキュリティ警告0件
  - Gate 3（mypy）: 全メソッドに型ヒント必須（`-> User`, `-> List[Post]`等）、SecretStr型チェック合格
  - Gate 4（git）: **3段階コミット戦略**（実装量考慮: 合計推定3h）
    - Commit 1: Pydanticモデル実装 (推定1h)
      - `feat: add Pydantic models (User, Post, Todo, Comment) with security constraints`
      - 対象: `utils/models.py`新規作成、4モデル実装、Field制約・frozen設定
      - 品質ゲート: ruff/mypy合格、カバレッジ+2.0%（モデル検証テスト含む）
    - Commit 2: JSONPlaceHolderClient実装 (推定1.5h)
      - `feat: add JSONPlaceHolderClient with Pydantic validation and security features`
      - 対象: `utils/api_client.py`変更、APIValidationError/6 CRUD methods/validation methods実装
      - 品質ゲート: ruff/mypy合格、カバレッジ+3.0%（統合テスト含む）
    - Commit 3: セキュリティテスト追加 (推定0.5h)
      - `test: add security tests for XSS prevention and SecretStr protection`
      - 対象: `tests/unit/test_api_client.py`変更、セキュリティテスト3件追加
      - 品質ゲート: pytest全合格、banditスキャン合格、カバレッジ+1.6%（セキュリティテスト）
  - セキュリティゲート: `bandit`スキャン合格（Critical/High脆弱性0件）、`.env`ファイルgitignore確認
...
#### ❌ よくある間違い [アンチパターン]
Task 1.3はPydanticモデル実装のため、「よくある間違い」ではなく「Pydantic導入効果」のみ記載（lines 655-666）
...
品質基準:
  - 型ヒント100%完備（mypy --strict準拠）
  - docstring全クラス・メソッドに付与
  - テストカバレッジ: Week 1目標26.4%達成（累積20→26.4%）
  - ruff/mypy完全合格
  - git commit: 1Task = 1commit（原子的タスク設計）
...
**1. ネスト記法ミス - env_nested_delimiter="__"忘れによるサイレント失敗 (頻度: HIGH, デバッグコスト: HIGH)**

```python
# ❌ 間違い: env_nested_delimiter設定忘れ
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_file=".env",
        # env_nested_delimiter="__" を忘れる
    )
    api: APIConfig = Field(default_factory=APIConfig)

# 環境変数 API__BASE_URL が認識されない
# → デフォルト値が使われる（エラーなし）
```

```python
# ✅ 正しい: env_nested_delimiter="__"設定
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_file=".env",
        env_nested_delimiter="__",  # 必須
    )
    api: APIConfig = Field(default_factory=APIConfig)

# 環境変数 API__BASE_URL → settings.api.base_url に正しくマッピング
```

**問題点**:
- サイレント失敗: エラーメッセージなし、デフォルト値が使われる
- デバッグコスト: 環境変数とコード両方を何度も確認（平均45分）
- 非自明性: Pydantic Settingsのドキュメント熟読が必要
- 影響範囲: API__BASE_URL等13箇所の全環境変数

**検出方法**: `test_settings_from_env_variables`で環境変数読み込みを確認

**実装箇所**: `config/settings.py:171`

---

**2. default_factory忘れ - mutable default argumentによるインスタンス共有 (頻度: MEDIUM-HIGH, デバッグコスト: HIGH)**

```python
# ❌ 間違い: default_factory使わない
class Settings(BaseSettings):
    api: APIConfig = APIConfig()  # 全インスタンスで共有される
    log: LogConfig = LogConfig()  # 全インスタンスで共有される

# インスタンス1で変更 → インスタンス2にも影響（予期しない動作）
settings1 = Settings()
settings1.api.timeout = 60
settings2 = Settings()
assert settings2.api.timeout == 60  # 意図せず変更されている
```

```python
# ✅ 正しい: default_factory使用
class Settings(BaseSettings):
    api: APIConfig = Field(default_factory=APIConfig)
    log: LogConfig = Field(default_factory=LogConfig)
...
settings1.api.timeout = 60
settings2 = Settings()
assert settings2.api.timeout == 30  # デフォルト値のまま
```

**問題点**:
- インスタンス共有: テスト独立性違反、マルチインスタンス環境で謎の共有状態
- デバッグコスト: 原因特定に1h+（Python根本理解必要）
- 非自明性: サイレント失敗、テスト実行順序で動作変化
- 影響範囲: APIConfig/LogConfig/TestConfig/SecurityConfig全て（4箇所）

**検出方法**: `test_settings_default_factory`で各インスタンスの独立性を確認

**実装箇所**: `config/settings.py:183-186`（4箇所）
...
##### アンチパターン1: モックの設定ミス - side_effect vs return_value混同

**評価メトリクス**:
- **頻度**: 50%（pytest mocking初心者、Week 1-4で頻発）
- **デバッグコスト**: 1-2h（AttributeError発生、mockの問題だと気づきにくい）
- **非自明性**: HIGH（エラーメッセージ "Mock object has no attribute..." 等、原因不明瞭）
- **教育価値**: HIGH（side_effectとreturn_valueの使い分け、mockingの本質理解）

**❌ 間違った実装**:
```python
def test_http_404_error_immediate_failure_wrong():
    """404エラーテスト - モック設定ミス版"""
    with patch.object(httpx.Client, 'request') as mock_request:
        mock_response = Mock()
        mock_response.status_code = 404

        # ❌ 間違い: raise_for_statusの戻り値をNoneに設定
        # 正解: side_effectに例外を設定すべき（例外をraiseするため）
        mock_response.raise_for_status.return_value = None  # 例外が発生しない
        mock_request.return_value = mock_response

        with pytest.raises(APIHTTPError):
            base_client.get("/invalid_user")  # ❌ 例外が発生しない
```

**🔴 発生するエラー**:
```
FAILED tests/unit/test_error_cases.py::test_http_404_error_immediate_failure_wrong

E   Failed: DID NOT RAISE <class 'utils.api_client.APIHTTPError'>
E
E   Instead, the test passed without raising any exception.
E   Mock object returned: <Mock spec='HTTPStatusError' id='...'>
```

**✅ 正しい実装**:
```python
def test_http_404_error_immediate_failure_correct():
    """404エラーテスト - 正しいモック設定版"""
    with patch.object(httpx.Client, 'request') as mock_request:
        mock_response = Mock()
        mock_response.status_code = 404

        # ✅ 正解: side_effectに例外を設定
        mock_response.raise_for_status.side_effect = httpx.HTTPStatusError(
            "Not Found", request=Mock(), response=mock_response
        )
        mock_request.return_value = mock_response
        ...
        assert "404" in str(exc_info.value)
        assert mock_request.call_count == 1  # リトライなし確認
```

**🎯 学習ポイント**:
- `return_value`: メソッド呼び出しの戻り値を設定（通常の返り値）
- `side_effect`: 例外やコールバック関数を設定（例外をraiseする場合）
- `raise_for_status()`が例外をraiseすべき場合は`side_effect`に例外を設定必須
- `return_value = None`では例外がraiseされない（正常終了とみなされる）
- mockingのデバッグは`print(mock_request.call_args_list)`で呼び出し履歴確認
- 複数回のリトライをモックする場合、`side_effect=[exception1, exception2, ...]`でリスト指定可能

---

##### アンチパターン2: pytest.raises()のスコープミス - with文の外で例外が発生

**評価メトリクス**:
- **頻度**: 40%（pytest初心者、Week 1-5で頻発）
- **デバッグコスト**: 30-60分（テストが通らない、スタックトレースが混乱を招く）
- **非自明性**: MEDIUM-HIGH（"DID NOT RAISE"と表示されるが、実際には例外が発生している）
- **教育価値**: HIGH（context managerの理解、例外処理のスコープ理解、pytestの動作理解）

**❌ 間違った実装**:
```python
def test_timeout_error_scope_mistake_wrong():
    """タイムアウトエラーテスト - スコープミス版"""
    with patch.object(httpx.Client, 'request') as mock_request:
        mock_request.side_effect = httpx.TimeoutException("Request timed out")

        # ❌ 間違い: pytest.raises()のwith文の外で例外が発生
        # 正解: with pytest.raises()内で例外を発生させる必要がある
        with pytest.raises(APITimeoutError) as exc_info:
            pass  # ❌ 何も実行していない

        # ❌ with文の外で実行 → 例外が捕捉されない
        base_client.get("/slow_endpoint")
```

**🔴 発生するエラー**:
```
FAILED tests/unit/test_error_cases.py::test_timeout_error_scope_mistake_wrong

tests/unit/test_error_cases.py:123: in test_timeout_error_scope_mistake_wrong
    base_client.get("/slow_endpoint")
E   utils.api_client.APITimeoutError: Request timed out after 3 retries

During handling of the above exception, another exception occurred:

_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _

>   raise RuntimeError(f"DID NOT RAISE {self.expected_exception.__name__}")
E   RuntimeError: DID NOT RAISE <class 'utils.api_client.APITimeoutError'>
```

**✅ 正しい実装**:
```python
def test_timeout_error_scope_correct():
    """タイムアウトエラーテスト - 正しいスコープ版"""
    settings = get_settings()
    with patch.object(httpx.Client, 'request') as mock_request, \
         patch('time.sleep') as mock_sleep:
        ...
        # ✅ 正解: pytest.raises()のwith文の中で例外を発生させる
        with pytest.raises(APITimeoutError) as exc_info:
            base_client.get("/slow_endpoint")  # ✅ with文の中で実行
        ...
        sleep_calls = [call[0][0] for call in mock_sleep.call_args_list]
        assert len(sleep_calls) == settings.api.retry_count - 1
```

**🎯 学習ポイント**:
- `pytest.raises()`はcontext manager（with文）として使用
- with文の**中**で例外が発生する必要がある（インデントレベルを確認）
- with文の外で例外が発生すると、pytestは捕捉できずテストが失敗
- `exc_info.value`でキャッチした例外オブジェクトにアクセス可能
- エラーメッセージの混乱: "DID NOT RAISE"と表示されるが、実際には例外が発生している（スコープ外）
- デバッグ方法: with文のスコープを確認、`print()`デバッグでwith文の実行を確認
- 複数のcontext managerをネストする場合、`with ... , \`で複数行記述可能
...
**実装効果**:
  ...
  - プロダクション品質のエラーハンドリング証明
  - **Anti-pattern学習効果**: 高頻度ミス2例（50%+40%の学習者が経験）でデバッグ時間3-4h削減
...
#### Task 1.6: README + pre-commit + docstring
...
テスト要件:
  - README検証テスト: test_readme_links_valid() [マークダウンリンク有効性検証、セットアップ手順の実行可能性確認]
  - pre-commit検証テスト: test_precommit_execution_time() [3秒以内実行確認、自動修正機能の動作検証]
  - docstring検証テスト: test_docstring_coverage() [全公開メソッドdocstring存在確認、Google/NumPy形式準拠検証]

カバレッジ達成計画:
  - Task 1.6貢献: 39.5% → 45% (+5.5%)
  - Week 7目標達成経路: Tasks 1.1-1.5 (39.5%) → Task 1.6 (45%) → Tasks 1.7-1.8 (60%)
  - 新規テスト: 30件追加 (内訳: docstring検証用単体テスト 25件、README/pre-commit検証テスト 5件)
  - 検証シナリオ: pytest --cov=. --cov-report=term-missing で45%達成確認

タスク分割戦略 (アトミック実装):
  **Task 1.6a: README.md + .env.example作成**
  - 推定時間: 2.0h
  - 成果物: README.md完成版、.env.exampleテンプレート
  - コミット戦略: `docs: add comprehensive README.md and .env.example template` (1コミット)
  - 品質ゲート: test_readme_links_valid() 合格、ruff/mypy合格

  **Task 1.6b: pre-commit setup**
  - 推定時間: 0.5h
  - 成果物: .pre-commit-config.yaml、pyproject.toml更新
  - コミット戦略: `chore: configure pre-commit hooks with ruff and mypy` (1コミット)
  - 品質ゲート: test_precommit_execution_time() 合格（3秒以内）、実行確認

  **Task 1.6c: docstring標準化**
  - 推定時間: 1.5h
  - 成果物: BaseAPIClient/JSONPlaceHolderClient/Settings docstring追加
  - コミット戦略: `docs: add Google-style docstrings to core classes` (1コミット)
  - 品質ゲート: test_docstring_coverage() 合格、カバレッジ45%達成

  **合計推定時間**: 4.0h (3h制約に対応するため、3サブタスクに分割)
...
**実装参照** (標準パターンは公式ドキュメント参照):
  - **README.md**: プロジェクト概要、技術スタック、クイックスタート、アーキテクチャ図を含む標準構成
  - **.env.example**: 4環境対応（development/testing/staging/production）、Pydanticネスト記法（`__`区切り）準拠
  - **.pre-commit-config.yaml**: ruff統合、3秒以内実行、重い処理はCI/CD委譲戦略
  - **pyproject.toml**: ruff/pytest/mypy統合設定、一元管理

**参考資料**:
  - README標準構成: https://github.com/matiassingers/awesome-readme
  - 12 Factor App設定管理: https://12factor.net/config
  - pre-commit公式: https://pre-commit.com/
  - Ruff設定: https://docs.astral.sh/ruff/configuration/

**Novel実装パターン（L3-L4）**:

**1. Docstring設計: 学習目標統合パターン**

BaseAPIClient/JSONPlaceholderClient/Settingsクラスに「学習目標」セクションを追加し、各メソッドの教育的価値を明示化。Google/NumPy docstring形式に準拠しつつ、ポートフォリオ特有の学習文脈を統合。
...
**カバレッジ達成**:
  - Task 1.6達成: 45% [Week 7中間マイルストーン]
  - カバレッジ増加: +5.5% (39.5% → 45%)
  - Week 7目標経路: Tasks 1.1-1.5 (39.5%) → Task 1.6 (45%) → Tasks 1.7-1.8 (60%)

**テスト数**: 累計55件 (既存25件 + 新規30件)

**アンチパターン（避けるべき実装）**:

**Anti-pattern 1: README不完全によるセットアップ失敗**
  - **問題**: 環境変数設定手順の欠落、依存関係インストールコマンドの不明確さ
  - **症状**: `ModuleNotFoundError`、`KeyError: 'API_KEY'`、セットアップに2h以上かかる
  - **頻度**: 50% (初心者が必ず遭遇)
  - **デバッグコスト**: 2h+ (エラー原因特定に時間がかかる)
  - **正しいアプローチ**:
    - `## ⚡ クイックスタート`セクションで環境変数設定を明示
    - `.env.example`の存在とコピーコマンドを明記
    - `uv sync`による依存関係インストールを最初に記載
  - **検証方法**: 新規環境で30分以内にセットアップ完了できるかテスト

**Anti-pattern 2: pre-commit設定ミスマッチによるコミット失敗ループ**
  - **問題**: `.pre-commit-config.yaml`と`pyproject.toml`のruff設定が不一致
  - **症状**: pre-commitは成功するがCIで失敗、またはその逆
  - **頻度**: 35% (pre-commit導入時に発生しやすい)
  - **デバッグコスト**: 1h+ (エラーメッセージが分かりにくい)
  - **正しいアプローチ**:
    - `pyproject.toml`を唯一の真実の情報源とする
    - `.pre-commit-config.yaml`では`args: ["--config=pyproject.toml"]`を明示
    - CI/CDパイプラインと同じlintコマンドをpre-commitで実行
  - **検証方法**: `uv run pre-commit run --all-files` と `uv run ruff check .` の結果が一致するか確認
...
**週次実装タスク**:
    - D7:Task 2.1: AsyncAPIClient実装 [async with、await対応]
    - D8:Task 2.2: 非同期テスト設計 [30テスト追加]
    - D9:Task 2.3: asyncio.gather()並行処理パターン習得
    - D10:Task 2.4:ロギング統合テスト実装 [structlog導入]
    - D11:Task 2.5:
    - D12:Task 2.6:
...
#### Task 2.1: AsyncAPIClient基礎実装 + Pydanticモデル統合
...
#### Task 2.2: AsyncAPIClient CRUD拡張実装
...
#### Task 2.6: Production Patterns実装 + W2完成
...
**週次実装タスク**:
    - D13:Task 3.1: pytest fixtureスコープ設計 [session/module/function]
    - D14:Task 3.2: ファクトリーパターン実装 [テストデータ生成]
    - D15:Task 3.3:Pydantic Settings実装 [型安全設定]
    - D16:Task 3.4:mypy型チェック導入
    - D17:Task 3.5:統合テスト基礎実装 [15テスト]
    - D18:Task 3.6:テスト自動実行CI基础設定
...
#### Task 3.1: Async基礎復習 + asyncio.gatHer()基礎 + **UserDataネストモデル導入**
...
#### Task 3.3: Async Context Manager + structlog統合
...
**日次実装タスク**:
  - D19:Task 4.1: エラー階層拡張実装
  - D20:Task 4.2: Exponential Backoff + Jitter実装
  - D21:Task 4.3: 中間確認 + テストカバレッジ戦略
  - D22:Task 4.4: エラーハンドリング統合 + structlog強化
  - D23:Task 4.5: W4仕上げ + W4振り返り
  - D24:Task 4.6: W4振り返り
...
#### Task 4.2: Exponential Backoff + Jitter実装
...
#### Task 4.4: エラーハンドリング統合 + structlog統合
...
**主要タスク**:
  - D25:Task 5.1: pytest fixture深掘り
  - D26:Task 5.2: Mock/PatcH基本
  - D27:Task 5.3: Pydantic Settings導入 + ConfigManager設計準備
  - D28:Task 5.4: ConfigManager基礎実装 [Settings基底、100行、12テスト]
  - D29:Task 5.5: ConfigManager拡張実装 [Security/Test設定、100行、13テスト]
  - D30:Task 5.6: APIClient統合 + 簡易振り返り [1H]
...
def test_multiple_config_loading():
    ...
    manager.load_config("security", {"api_key": "secret", "rate_limit_requests": 100, "rate_limit_window": 60})  # pragma: allowlist secret
...
**主要タスク**:
  - D31:Task 6.1: W1-2復習統合 + 概念図作成
  - D32:Task 6.1: W3-4復習統合 + 非同期パターン図
  - D33:Task 6.1: W5-6復習統合 + テスト設計図
  - D34:Task 6.1: 統合自律チャレンジ PHase 1   - API調査 + 基本設計
  - D35:Task 6.1: 統合自律チャレンジ PHase 2   - Sync/Async実装
  - D36:Task 6.1: 統合自律チャレンジ PHase 3   - テスト完成 + README
...
### D34-36 (Thu.- Sat.): 統合自律チャレンジ [20H, 3日間]
...
**週次目標**:
  ...
  - **staging環境手動Smoke Test** [docker-compose up + Health Check確認]（Week 8で自動化）
  - BuildKit最適化 [初回70s, 再ビルド25s達成]
  - .dockerignore最適化 [build context 90%削減: 150MB→15MB]
  - カバレッジ85%維持 [W6から継続、python追加なし]

**主要タスク**:
  - D37:Task 7.1: Docker基礎 + AI template生成workflow習得
  - D38:Task 7.1: docker-compose 4環境 + .dockerignore最適化 + **staging環境手動Smoke Test**
  - D39:Task 7.1: 4-stage Dockerfile + Layer cache最適化
  - D40:Task 7.1: docker-compose 4環境完成
  - D41:Task 7.1: Testing + coverage 85%維持確認
  - D42:Task 7.1: Buffer/review + W7振り返り

**ポートフォリオ成果物**:
  - [ ] Docker 4-stage Dockerfile実装 [イメージサイズ60%削減: 1.2GB→480MB]
  - [ ] docker-compose 4環境構築 [環境分離完全実装]
  - [ ] **staging環境Infrastructure Baseline確立** [docker-compose.staging.yml動作確認、Health Check合格]
...
#### D38 [.Tue]: docker-compose 4環境構築

**総学習時間**: 10H
**カバレッジ目標**: 85% [維持]

##### Task 7.2: docker-compose.yml 4環境実装 + staging環境手動検証
...
**要件**:
  ...
  - .dockerignore最適化
  - ビルド時間短縮確認
  - **staging環境手動Smoke Test**（Week 8で自動化）
...
PHase 1: AI説明・概念理解
  ...
  - **Deployment Gate概念理解** [AI説明 → Week 8実装準備]
...
PHase 2: AI協働実装 [7H]
  ...
  - **staging環境手動Smoke Test** [docker-compose up確認 + curl health check]
...
**チェックポイント**:
  ...
  - [ ] .dockerignore最適化完了
  - [ ] ビルド時間20%以上短縮確認
  - [ ] **staging環境手動Smoke Test完了** [docker-compose up + curl health check]
  - [ ] **docker-compose staging環境起動確認成功** [HTTP 200応答確認]
  - [ ] **Manual Test Report作成完了** [tests/system/manual_test_report_d38.md]
  - [ ] **手動テストステップ記録完了** [curlコマンド、期待結果、実行結果記録]
  - [ ] docker-compose理解度40%達成
  - [ ] **Week 8 System Tests実装準備完了** [Deployment Gate概念理解]

**カバレッジ目標**: 85% [維持]

**Week 8への引き継ぎ事項**:
  - docker-compose.staging.yml動作確認済み → Week 8でSystem Tests自動化
  - Health endpoint URL: http://localhost:8001/health
  - Manual test cases documented → pytest自動化対象明確化
...
#### D40 [.Thu]: docker-compose 4環境完成 + 本番環境最適化
...
**達成状況**:
  ...
  - ✅ D38: docker-compose 4環境構築 + staging環境手動Smoke Test完了
  - ✅ D39: Docker 4-stage Dockerfile実装完了
  - ✅ D40: docker-compose 4環境完成 + 本番環境最適化完了
  - ✅ D41: Dockerネットワーク・ボリューム設定 + カバレッジ85%維持
  - ✅ D42: W7振り返り + Docker総合演習完了
  - ✅ カバレッジ85%維持 [W6から継続]
  - ✅ Docker 4-stage実装 [イメージサイズ60%削減: 1.2GB→480MB]
  - ✅ docker-compose 4環境 [dev/test/demo/prod]
  - ✅ staging環境Infrastructure Baseline確立 [手動Smoke Test合格、Week 8で自動化]
  - ✅ BuildKit最適化 [ビルド時間50%削減]
...
**Week 7完了Acceptance Criteria（受け入れ基準）**:
  - [ ] docker-compose.staging.yml動作確認成功（HTTP 200応答）
  - [ ] Health endpoint URL確認: http://localhost:8001/health
  - [ ] Manual Test Report作成完了（tests/system/manual_test_report_d38.md）
  - [ ] 手動テストステップ記録完了（curlコマンド、期待結果、実行結果記録）
  - [ ] .env.staging環境変数設定確認完了
  - [ ] Week 8 System Tests実装準備完了（Deployment Gate概念理解）

**Week 8への引き継ぎ事項**:
  - Infrastructure Layer構築完了（Docker基盤確立）
  - 手動Smoke Test実施結果記録済み → Week 8で自動化対象明確化
  - staging環境docker-compose設定確認済み → Week 8でSystem Tests実装
...
**主要タスク**:
  - D43:Task 8.1: GitHub Actions基礎 + テストworkflow実装 [7H]
  - D44:Task 8.1: 品質ゲート自動化 + CI/CD最適化強化 [7H]
  - D45:Task 8.1: キャッシュ戦略最適化 [7H]
  - D46:Task 8.1: セキュリティスキャン統合 [safety + CodeQL] [7H]
  - D47:Task 8.1: CI/CDバッジ追加 + README更新 [7H]
  - D48:Task 8.1: W8振り返り + CI/CD総合演習 [7H]
...
##### Task 8.6: Parallel Job最適化 + Slack通知統合
...
#### **D47 [.Fri]: CI/CDバッジ追加 + README更新   - 7H** {#day47}
...
#### 📚 学習サマリー
  ...
  - **ポートフォリオポートフォリオ成果物**:
    ...
    - [ ] デモ環境準備 [docker-compose staging環境で3分実行シナリオ確立]
...
**デモ環境準備 [3分実行シナリオ]**:
1. **docker-compose staging環境活用**:
     - 既存のstaging環境を活用 [新規実装なし]
     - 3分以内で技術力を証明できる実行シナリオ確立

2. **デモシナリオ設計**:
     - ステップ1: `docker-compose up staging`で環境起動 [30秒]
...
**AI依頼テンプレート [デモシナリオ設計]**:
```
「docker-compose staging環境を活用した3分実行シナリオを設計してください:

**条件**:
  - 既存のstaging環境を使用 [新規実装なし]
  - 3分以内で以下の技術力を証明:
    - async/await並行処理
    - エラーハンドリング・リトライロジック
    - structlog構造化ログ出力
...
### D56 （Tue.）: AI品質検証ゲート構築 + 応募文面準備 [7H、実質4.7H]
...
### D57 （Wed.）: 超高品質応募実行 [10件、完全手動、17分/件]
...
### D58 （Thu.）: 高品質応募実行 [20件、AI+厳密検証、12分/件]
...
### D59 (Fri.): 中品質応募実行 [16件、AIテンプレート、8分/件]
