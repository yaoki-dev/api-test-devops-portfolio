...
1. 既定でリトライを許可するメソッドを `GET`、`HEAD`、`DELETE`、`OPTIONS`、`TRACE` に限定する。`PUT` はHTTPの意味上は冪等でも、実サーバーの実装差を安全側に扱うため既定では再送しない。
   （2026-09-22 追記）`DELETE` と `PUT` を非対称に扱う理由: `DELETE` は対象の識別子だけを送り、再送しても「対象が無い」で収束しやすい。`PUT` は本文で資源を上書きするため、部分更新として実装されたサーバーや、更新日時・バージョン番号を本文から採るサーバーでは再送が観測可能な差分を生む。この集合は「冪等なメソッド」ではなく「既定で再送するメソッド」であり、コード上の定数名も `DEFAULT_RETRY_METHODS` とする。
