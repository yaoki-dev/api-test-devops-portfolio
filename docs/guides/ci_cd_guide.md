...
graph TB
    ...
    style PM fill:#2ed573,color:#fff
...
全てのTrivyスキャン（PR filesystem/image、Post-Merge）で共通のComposite Actionを使用することで、DRY原則を実践しメンテナンス性を向上させています（72行の重複コード→7行の呼び出しに削減）。
...
- name: Verify filesystem scan execution
  ...
  with:
    sarif-file: 'trivy-fs-results.sarif'
...
| 2 | サイズ ≥100 bytes | 空ファイル、部分書き込み | `stat` + 閾値比較 |
...
4. **Composite Action活用**: 3箇所（PR filesystem/image、Post-Merge）で同一ロジック共有
...
| 品質基準 | 目標値 | 現在値 | 検証コマンド |
|---------|-------|-------|------------|
| カバレッジ | 85% | 76.40% | `pytest --cov-fail-under=76` |
...
# SARIF検証
jq . trivy-fs-results.sarif | head -20

# ファイルサイズ確認
ls -lh trivy-fs-results.sarif
```
...
```yaml
- name: Verify image scan execution
  if: always() && steps.image-scan.conclusion != 'skipped'  # skip状態を正しく検出✅
```
