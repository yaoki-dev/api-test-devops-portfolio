...
6. [git-workflow活用ガイド](#git-workflow活用ガイド)
7. [優先度別推奨順位表](#優先度別推奨順位表)
...
| コマンド | 機能 |
|----------|------|
| `/feature <name>` | featureブランチ作成 |
| `/release <version>` | releaseブランチ作成 |
| `/hotfix <name>` | hotfixブランチ作成 |
| `/flow-status` | Git Flow状態確認 |
| `/clean-gone` | [gone]ブランチクリーンアップ |

> **Note**: ブランチ完了には `/superpowers:finishing-a-development-branch` を使用
...
### 6. documentation-generator（ドキュメント生成）
...
### 7. performance-optimizer（パフォーマンス最適化）
...
git-workflowはGit Flowワークフローを完全サポートするプラグインです。
...
| 順位 | プラグイン | 理由 |
|------|-----------|------|
| 1 | git-workflow | ブランチ管理 |
| 2 | pr-review-toolkit | PR品質向上 |
| 3 | testing-suite | テスト自動化 |
