...
| **`例外チェーン方針`** | 不変条件は「外部の実在API（GitHub）のレスポンス由来例外を、未サニタイズのまま cause にしない」。満たし方は2通りで、`from None` によるチェーン切断（12箇所）と、サニタイズ済み代理 cause へのチェーン（`SanitizedJSONDecodeError`）。合成データのモックAPI（JSONPlaceholder）は対象外で `from e` を維持。 | Pydantic の `ValidationError` は検証失敗時の入力値を保持するため、`from e` で連結すると Sentry の stacktrace frame vars 経由で PII が到達しうる（スクラブはキー名ベースで、機密キー集合に無い名前の値は素通しする）。ただし「切る」一択にすると JSON パース失敗時の原因追跡を失うため、`utils/github_error_handler.py` では `reason` と位置情報だけを詰め替えた代理例外を作って `from` に渡し、本文を捨てつつデバッグ性を残している。実装の対称性ではなく「データの出所によるリスク」を一貫性の軸に置いた判断で、代償として同種処理の実装が非対称になる。 |
