...
> **Python/Docker/CI/CDを統合したAPIテスト自動化ポートフォリオ。（現）215件のテストで品質を保証。**

## 概要

- **（現）215件のテストスイート**: Unit / Integration / Security / Performance
- **カバレッジ 65%**: 継続的な品質向上
...
> **📝 デモ内容**: クイック実行例（基本テスト19件、デモ時間短縮のため抽出。全215件は約60秒）
> **🔍 全215件を今すぐ確認**: [GitHub Actions CI/CD](https://github.com/yuta158/api-test-portfolio/actions) でフルテスト結果＋カバレッジレポートを閲覧

**何がわかるか**:
- pytest + pytest-covによる自動テスト実行
- カバレッジレポートによる品質可視化
- テスト実行: 基本19件 ~5秒、全215件 ~60秒

<details>
<summary>全テスト実行コマンド（215件、約60秒）</summary>
...
| `release/*` | リリース準備・最終調整 | main + develop |
...
```mermaid
graph TB
    subgraph "Test Pyramid - 215件（3層 + マーカー）"
        E2E["🔝 E2E<br/>0件（Playwright導入予定）"]
        Integration["🔗 Integration<br/>12件 (5.6%)"]
        Unit["🧱 Unit<br/>112件 (52.1%)"]
        Markers["🏷️ Security: 29件 | Performance: 5件 | Validation: 14件 | Regression: 10件"]
    end
...
> **Note**: pytestパラメータ化テストにより、実テスト関数182件 → pytest収集215件
