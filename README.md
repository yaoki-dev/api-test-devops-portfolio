...
- **`1,528件のテスト`**（2026-08-27 `pytest --collect-only --no-cov` 実測）: Unit(1,504) / Integration(15, うちExternal 4件含む) / Performance(7, 週次のみ) / Smoke(2)
- **`カバレッジ: 97.64%`**（2026-08-27 `unit+integration` かつ `not external` 実測）: 継続的な品質向上
- **`CIカバレッジ対象テスト: 1,515件`**（`unit or integration` かつ `not external`。external・performance・smoke除外。PR ValidationはこれにSmoke 2件をカバレッジ計測外で追加実行）
  - 内訳: Unit 1,504件 + Integration 11件（15件のうちexternal 4件を除外）
